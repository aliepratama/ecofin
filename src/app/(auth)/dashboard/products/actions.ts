...
export async function addProduct(formData: FormData) {
  ...
  const isOperasional = formData.get("isOperasional") === "true";

  let type: "BAHAN_BAKU" | "OPERASIONAL" | "MENU" = "MENU";
  if (isRawMaterial) type = "BAHAN_BAKU";
  if (isOperasional) type = "OPERASIONAL";
  ...
  let finalName = name;
  if (isRawMaterial) finalName = `[Bahan Baku] ${name}`;
  else if (isOperasional) finalName = `[Operasional] ${name}`;
  ...
  await db.insert(products).values({
    ...
    unit:
      unit ||
      (isRawMaterial ? "Kg/Liter" : isOperasional ? "Bulan/Lembar" : "Porsi"),
    type,
    aiRecipe,
  });
